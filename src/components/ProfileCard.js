  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: '1.5rem', alignItems: 'start' }}>
        ...
        {/* Avatar */}
        <div style={{ width: 72, height: 72, borderRadius: '50%', overflow: 'hidden', border: '2px solid var(--border)', flexShrink: 0 }}>
            ...
            <>
              ...
              <div style={{ display: 'flex', height: 6, borderRadius: 100, overflow: 'hidden', gap: 1 }}>
                {profile.languages.map(l => (
                  <div key={l.name} style={{ flex: l.pct, background: l.color }} title={`${l.name}: ${l.pct}%`} />
                ))}
              </div>
